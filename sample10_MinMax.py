...
for end_date in end_data_list:
    ...
    for code in tqdm(stock_list['종목코드'].values):
        ...
        for r in range(1000000):
            if r+1 == len(x):
                break
            x_data.append(mm.transform(x[r:r+1]))
            y_data.append(x[r+1])

        x_data = np.array(x_data)
        y_data = np.array(y_data)
        # x_public = data.iloc[-1].to_numpy().reshape(1,1,5)  # 2021년 11월 1일부터 11월 5일까지의 데이터를 예측할 것이다.
        x_public = data.iloc[-1].to_numpy().reshape(1,5)
        x_public = mm.transform(x_public)
        x_public = x_public.reshape(1,1,5)
        model = RNN().to(device)
