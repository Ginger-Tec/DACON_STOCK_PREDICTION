...
for end_date in end_data_list:
    ...
    for code in tqdm(stock_list['종목코드'].values):
        ...
        data = pd.pivot_table(data=data, values=['Close','Open'], columns='weekday', index='weeknum')
        ...
        y = data.iloc[1:-1,0:5].to_numpy()  # 2021년 1월 11일 ~ 2021년 10월 29일까지의 데이터를 학습한다.
        x_public = data.iloc[-2].to_numpy().reshape(1,10)  # 2021년 11월 1일부터 11월 5일까지의 데이터를 예측할 것이다.
        model =ANN().to(device)
